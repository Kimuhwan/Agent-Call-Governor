...
| 프로필 | 적합한 작업 | 동작 |
| --- | --- | --- |
| `strict` | 되돌리기 쉽고 위험이 낮으며 지연시간이 중요한 작업 | 작은 예산, 단 고위험 작업은 전략을 바꾼 재시도 1회 보장 |
...
버전 0.1.0은 **프롬프트 수준 거버넌스와 선택적 결정론적 강제 실행** 단계입니다. 스킬은 위임 전에 Codex의 판단을 안내하고, `governor.py`는 호출자가 명시적으로 실행할 때만 결정을 강제합니다.

아직 모든 Codex 또는 SDK 호출을 자동 차단하거나, 신뢰 가능한 런타임 로그를 자동 수집하거나, 완전한 agent-call firewall로 동작하지는 않습니다. 런타임 로깅과 호출 전·후 연동이 다음 개발 단계입니다.
