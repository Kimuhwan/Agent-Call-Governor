...
Codex를 다시 시작하세요. 스킬은 `~/.codex/skills/agent-call-governor`에 복사됩니다.
...
SQLite가 권위 있는 원장이며 JSONL은 선택형 미러 또는 내보내기 형식입니다. 기본 이벤트에는 원문 프롬프트, material input, 도구 인자, 도구 결과, transcript, 예외 메시지가 들어가지 않습니다. fingerprint, 수명주기 단계, 결정, 진행도, 소요 시간, 명시적으로 안전한 메타데이터만 기록합니다.
...
자동 수명주기 기록은 [Codex 훅 안내](agent-call-governor/references/codex-hooks.md)와 [hooks.json 예제](examples/codex-hooks.json)를 따라 `PreToolUse`, `PostToolUse`, `SubagentStart`, `SubagentStop`을 설정하세요.
...
- `GovernedRunner`: 전체 `Runner.run`, `Runner.run_sync` 워크플로를 애플리케이션 소유 경계에서 강제
...
- fingerprint는 저장 내용을 줄이는 식별자이지 암호화가 아닙니다. 원장을 내보내기 전에 안전한 메타데이터인지 검토하세요.
